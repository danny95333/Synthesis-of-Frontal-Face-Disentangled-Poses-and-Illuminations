...
class Siamese(nn.Module):
    def __init__(self):
        ...
        #-------------------------------------------decoder-------------------------------------------------------------------
        #240x6x6
        self.dconv52 = nn.Sequential(nn.Conv2d(240, 160, kernel_size=3, stride=1, padding=1),
                                     nn.BatchNorm2d(160),
                                     nn.ELU(True))
        #160x6x6
        self.dconv51 = nn.Sequential(nn.Conv2d(160, 256, kernel_size=3, stride=1, padding=1),
                                     nn.BatchNorm2d(256),
                                     nn.ELU(True))
        #256x6x6 bilinear interpolation upsamping
        #self.upsampling43 = nn.UpsamplingBilinear2d(scale_factor=2)
        #256x12x12
        self.dconv43 = nn.Sequential(nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1),
                                     nn.BatchNorm2d(256),
                                     nn.ELU(True))
        #256x12x12
        self.dconv42 = nn.Sequential(nn.Conv2d(256, 128, kernel_size=3, stride=1, padding=1),
                                     nn.BatchNorm2d(128),
                                     nn.ELU(True))
        #128x12x12
        self.dconv41 = nn.Sequential(nn.Conv2d(128, 192, kernel_size=3, stride=1, padding=1),
                                     nn.BatchNorm2d(192),
                                     nn.ELU(True))
        #192x12x12 bilinear interpolation upsamping
        #self.upsampling33 = nn.UpsamplingBilinear2d(scale_factor=2)
        #192x24x24
        self.dconv33 = nn.Sequential(nn.Conv2d(192, 192, kernel_size=3, stride=1, padding=1),
                                     nn.BatchNorm2d(192),
                                     nn.ELU(True))
        #192x24x24
        self.dconv32 = nn.Sequential(nn.Conv2d(192, 96, kernel_size=3, stride=1, padding=1),
                                     nn.BatchNorm2d(96),
                                     nn.ELU(True))
        #96x24x24
        self.dconv31 = nn.Sequential(nn.Conv2d(96, 128, kernel_size=3, stride=1, padding=1),
                                     nn.BatchNorm2d(128),
                                     nn.ELU(True))
        #128x24x24 bilinear interpolation upsamping
        #self.upsampling23 = nn.UpsamplingBilinear2d(scale_factor=2)
        #128x48x48
        self.dconv23 = nn.Sequential(nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),
                                     nn.BatchNorm2d(128),
                                     nn.ELU(True))
        #128x48x48
        self.dconv22 = nn.Sequential(nn.Conv2d(128, 64, kernel_size=3, stride=1, padding=1),
                                     nn.BatchNorm2d(64),
                                     nn.ELU(True))
        #64x48x48
        self.dconv21 = nn.Sequential(nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1),
                                     nn.BatchNorm2d(64),
                                     nn.ELU(True))
        #64x48x48 bilinear interpolation upsamping
        #self.upsampling13 = nn.UpsamplingBilinear2d(scale_factor=2)
        #64x96x96
        self.dconv12 = nn.Sequential(nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1),
                                     nn.BatchNorm2d(64),
                                     nn.ELU(True))
        #64x96x96
        self.dconv11 = nn.Sequential(nn.Conv2d(64, 32, kernel_size=3, stride=1, padding=1),
                                     nn.BatchNorm2d(32),
                                     nn.ELU(True))
        #32x96x96
