...
parser.add_argument('--beta1', type=float, default=0.7, help='beta1 for adam. default=0.9')
parser.add_argument('--cuda', action='store_true', help='enables cuda')
parser.add_argument('--outf', default='/home/shumao/wyw_files/siamese_output_2nd_sameW', help='folder to output images and model checkpoints')
...
parser.add_argument('--save_step', type=int, default=1000, help='save weights every 10000 iterations ')
...
saveFile = open('/home/shumao/wyw_files/siamese_output_2nd_sameW/record.txt', 'w')

opt = parser.parse_args()
print(opt) # print every parser arguments
# print(opt.niter)
saveFile.write("niter:" + str(opt.niter) + "\n")
saveFile.write("---lr:" + str(opt.lr) + "\n")
saveFile.write("beta1:" + str(opt.beta1) + "\n")
saveFile.write("W:-1-1-1-1-1-1-" + "\n")
saveFile.write("second model" + "\n")
try:
...
optimizerS = torch.optim.Adam(netS.parameters(),lr=opt.lr, betas=(opt.beta1, 0.999))
...
for iteration in range(1,opt.niter+1):
    ...
    try:
        ...
        images_1,po_1,li_1,GT_1,by_image,same_po,same_li,same_id = data_train_1.next()
    # try:
    #     images_2, iden_2, po_2, li_2,GT_2 = dataset_train_2.next()
    # except StopIteration:
    #     dataset_train_2 = iter(loader_train_2)
    #     images_2,iden_2,po_2,li_2,GT_2= dataset_train_2.next()
    #!!!!!!How to get GT
    GT.data.resize_(GT_1.size()).copy_(GT_1)
    ...
    err_total = 1 * err_recon + 1 * err_contraL + 1 * err_contraP + 1 * err_contraI + 1 * err_classP + 1 * err_classL
